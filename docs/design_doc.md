...
既存のGodot GDExtensionプロジェクトを拡張し、**C++による「コアシステム (データ・ロジック)」** と、**Godotシーンによる「ビュー (表現・操作)」** を完全に分離することで実現します。

...
| P4-09 | P2 | `generator_regression_batch_b_implementation` | Done | `tests/test_generators.py`, `tests/fixtures/`, `reports/p4_batch_b_generator_regression_impl_report.md` | GTC-04〜GTC-07 が実装され、既存 GTC-01〜GTC-03 を壊さない |

---

## 5. 直近実行順（推奨）

1. `spec_input_normalization` の最小実装タスクを起票
2. 共通化候補 C-01/C-02 の PoC タスクを起票

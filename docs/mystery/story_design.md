...
| フラグID           | 初期値 | 設定トリガー            | 用途                                   |
| ------------------ | ------ | ----------------------- | -------------------------------------- |
| `talked_to_tanaka` | false  | 田中との初会話終了      | 二回目会話の分岐、足跡調査の前提条件   |
| `talked_to_sato`   | false  | 佐藤との初会話終了      | 二回目会話の分岐                       |
| `talked_to_suzuki` | false  | 鈴木との初会話終了      | 二回目会話の分岐                       |
| `talked_to_kenta`  | false  | ケンタとの初会話終了    | 二回目会話の分岐                       |
...
## 5. シナリオ進行の高度化 (Phase 38 追加仕様)

### 会話のバリエーション (Dialogue Branching)
各NPCとの会話において、`talked_to_[id]` フラグを使用して分岐を実装。初対面時と二回目以降でセリフが変化し、世界観の深みを演出。

### 調査完了のヒントシステム (Exit Hint System)
倉庫から脱出しようとした際に証拠が不足している場合、未入手のアイテムカテゴリに応じた具体的なヒント（足跡を探すべき、霊的な残滓を探すべき等）を表示。
- **実装レベル**: `mystery.yaml` の `hs_exit` クリック時アクション内で `if_has_items` をネストして判定。

...
詳細は [`docs/testing_strategy.md`](../docs/testing_strategy.md) を参照。

### テスト種別と実行コマンド

| Tier | コマンド | 所要時間 | 目的 |
|---|---|---|---|
| **smoke** | `./dev.sh smoke` | ~5-10 秒 | 起動・必須ノード・プロローグ・ロケール確認 |
| **e2e** | `./dev.sh e2e` | ~60 秒 | 全シナリオ（調査→推理→対決→エンディング）通し |
| **monkey** | `./dev.sh monkey` | ~30-60 秒 | ランダム操作でクラッシュ・デッドロックを検出 |
...
### monkey テスト失敗時の再現方法
```bash
# 失敗時は seed=XXXXXX を控えて固定シードで再実行
MONKEY_SEED=XXXXXX ./dev.sh monkey
```
